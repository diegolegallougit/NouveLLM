...
export default function Sidebar({ onSelectConversation, activeConversationId, onNewConversation, refreshKey, userRole, onClose, inDrawer }: SidebarProps) {
  ...
  const filtered = inDrawer && searchQuery
  ...
  return (
    ...
    >
      ...
      {inDrawer && (
        <div className="px-3 pt-3 pb-2.5 border-b border-[#D8D8D8] flex-shrink-0">
      ...
      <div className="flex-1 overflow-y-auto nl-scroll py-1">
        {loading ? (
          ...
          >
            {inDrawer && searchQuery ? 'Aucun résultat.' : 'Vos conversations apparaîtront ici.'}
          </p>
        ) : inDrawer ? (
      ...
      {/* Drawer footer — Dossiers + Séances (non-STUDENT) + Paramètres (all) */}
      {inDrawer && (
